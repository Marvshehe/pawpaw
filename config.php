<?php
/**
 * Marv's Burgers server configuration
 * ------------------------------------------------
 * IMPORTANT SECURITY:
...
$PAYMONGO_SECRET_KEY = getenv('PAYMONGO_SECRET_KEY') ?: 'sk_live_tCwh1ZfdrLU6ZAAC11aHQmpA';
$PAYMONGO_PUBLIC_KEY = getenv('PAYMONGO_PUBLIC_KEY') ?: 'pk_live_uXQVvXN2NiMWJg3ukU9JFyx7';
...
$MAIL_FROM_NAME = getenv('MAIL_FROM_NAME') ?: "Marv's Burgers";
...
function send_sale_notification($to, $subject, $htmlBody, $fromEmail = null, $fromName = null) {
    ...
    if ($SMTP_ENABLED && $SMTP_USER && $SMTP_PASS) {
        ...
        error_log("[MarvsBurgers] SMTP failed: " . $mailer->getLastError());
    }
...
<html><body style="font-family:Arial,sans-serif;background:#111;color:#eee;padding:24px;">
  <div style="max-width:520px;margin:0 auto;background:#1a1a1a;border:1px solid #333;border-radius:12px;padding:28px;">
    ...
    <p style="color:#666;font-size:12px;margin-bottom:0;">Marv's Burgers - automatic sales notification</p>
  </div>
</body></html>';
}
